...
;
; The first two byte received from the UART are the number of bytes to follow (LSB first)
; The program is downloaded into address RAMSTART and then JMPed to.
;
; If P1.0 is high, it boots from flash:
;
; Sector 0 - 256 bytes in length - is loaded into address RAMSTART and JMPed to
;
; Because this program resides in RAM, we don't bother redirecting interrupts - the OS can set up the interrupt vectors
; (overwriting this bootloader program) as and when it needs them.
;
; The ReadBytes routine used in this program uses a data stack, pointed to by R0, for passing parameters. This is because this
; routine is copied and pasted from a library used by the MC language, and this method of parameter passing is a convention
; used by the current implementation of MC.
...
START:
	...
	; P1.2 - Output. #SS select for the display SPI interface. Bring low to start a new communications frame.
	; P1.3 - Input. AT keyboard data.
	; P1.4 - Input. SPI serial data in.
	; P1.5 - Output. SPI serial data out. (Used for both flash memory and display)
	; P1.6 - Output. SPI serial clock. (Used for both flash memory and display)
	; P1.7 - Output. #S select for flash memory.
	; P3.0 - UART RXD
	; P3.1 - UART TXD
	...
	; P1.2: Set display #SS high
	; P1.3: Input
	; P1.4: Input
	; P1.5: Output
	; P1.6: Output
	; P1.7: set flash #S high
	mov p1,#10011101b  
